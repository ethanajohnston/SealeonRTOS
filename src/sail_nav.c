...
enum status_code NAV_CalculateRudderPosition(float course, float heading, float *rudder_angle, struct PID rudderPID)
{
	// Return if any of the pointers are NULL
	...
	rudderPID.SetPoint = course; // where we want to go or goal 
	
	// Compute the rudder angle
	*rudder_angle = MATH_ForceAngleTo180(update(rudderPID, heading)); //heading is where boat is going right now
